...
class ImplicitIntervalTreeWithInterpolationIndex[V] extends BaseIntervalHolder[V] {
  ...
  override def overlappers(start: Int, end: Int): util.Iterator[BaseNode[V]] = {
    ...
    val prediction: Int = predict(start)
    if (prediction == invalidIndex) {
      return simpleOverlap(start, end, results)
    }
    ...
    while (subtree != rootIndex && (subtree >= internalTree.size || start < this.internalTree(subtree).outsideMaxEnd || outsideMinStart(subtree, k) < end)) {
    ...
    scan(subtree, k, start, end, results)
    results.iterator
  }
  ...
  private def scan(index: Int, level: Int, start: Int, end: Int, results: ArrayBuffer[BaseNode[V]]): Unit = {
    ...
    if (index >= this.internalTree.size) {
      if (level > 0) {
        scan(left(index, level), level - 1, start, end, results)
      }
      return
    } else if (level <= 2) {
      ...
      breakable {
        while (rIndex <= rightMostLeafIndex) {
          if (this.internalTree(rIndex).getStart >= end) {
            break()
          }
          if (this.internalTree(rIndex).getEnd > start) {
    ...
    if (node.insideMaxEnd > start) {
      val ck: Int = level - 1
      scan(left(index, level), ck, start, end, results)
      val nStart = this.internalTree(index).getStart
      if (nStart < end) { // this node isn't already past query
        if (this.internalTree(index).getEnd > start) { // this node overlaps query
          results.append(this.internalTree(index))
        }
        scan(right(index, level), ck, start, end, results);
      }
    }
  }
